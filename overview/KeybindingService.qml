...
Item {
    ...
    function bindingScript(optimized, ownerToken) {
        const commands = [
            ...
            'hl.layer_rule({ name = "super-switcher-instant", match = { namespace = "^(omarchy-super-switcher|omarchy-orbit-handoff)$" }, no_anim = true, animation = "none" })',
        ...
        commands.push('hl.bind("ALT + TAB", hl.dsp.global("omarchy-super-switcher:next"), { repeating = true, description = "Orbit next window" })');
        commands.push('hl.bind("ALT + SHIFT + TAB", hl.dsp.global("omarchy-super-switcher:previous"), { repeating = true, description = "Orbit previous window" })');
