...
Item {
  ...
  function completeWindowQuery(text) {
    ...
    try {
      ...
      console.warn("super-switcher: unable to read current window order:", error)
      return
    }
  ...
  function open(payloadJson) {
    ...
    if (payloadJson) {
      try {
        ...
        console.warn("super-switcher: invalid invocation payload:", error)
      }
    }
  ...
  function completeSnapActive(text) {
    ...
    try {
      ...
      console.warn("super-switcher: unable to read active window for snap:", error)
    }
  ...
  function completeSnapMonitors(text) {
    ...
    try {
      ...
      console.warn("super-switcher: unable to read monitors for snap:", error)
    }
  ...
  function completeSnapClients(text) {
    try {
      ...
      console.warn("super-switcher: unable to read clients for snap:", error)
      root.snapClientRows = []
    }
  ...
  function maybeOpenSnapManager() {
    ...
    if (!root.snapTargetWindow || !root.snapTargetMonitor) {
      console.warn("super-switcher: snap layouts need an active window and monitor")
      return
    }
  ...
  function requestActivationReadiness() {
    if (!root.activationCommitInProgress || root.activationCommitFinalizing || root.activationCommitAttempts === 0 || !root.pendingWindow || activationDispatch.running || activationReadinessQuery.running)
      return
    if ((root.activationTargetSurfaceReady && root.activationFocusConfirmed) || (root.activationCommitSettling && root.activationReadiness.startsWith("fallback-")) || root.activationReadiness.startsWith("fallback-"))
      return
  ...
  function abortActivationCommit(targetClosed) {
    ...
    if (!targetClosed)
      console.warn("super-switcher: selected window did not accept focus; keeping Orbit open")
  ...
  function finalizeActivationCommit() {
    ...
    // 切换落点全屏策略（查实时状态，绝不重复触发开关）：焦点确实落在目标上
  ...
  function advanceActivationCommit(fromReadiness) {
    ...
    if (root.handoffNeedsCover && !root.activationCommitSettling) {
      ...
      else if (root.activationReadiness === "fallback-unavailable" || root.activationReadiness === "fallback-unsupported")
        activationRevealTimer.restart()
      else
  ...
  function observeActivationTargetCommit(text, generation) {
    ...
    try {
      ...
      // Stock Hyprland has no `orbit-window-ready` dispatcher, so the readiness
      // query can never succeed. Finish via the fallback path immediately
      // instead of stalling until the 1600 ms settle timeout.
    ...
    Scope {
      ...
      LazyLoader {
        ...
        PanelWindow {
          ...
          color: "transparent"
          exclusionMode: ExclusionMode.Ignore
          WlrLayershell.namespace: "omarchy-super-switcher"
          WlrLayershell.layer: WlrLayer.Overlay
          WlrLayershell.keyboardFocus: WlrKeyboardFocus.Exclusive
